...
# Creating an instance of CentroidTracker to use in main while loop.
# In order not to get an error in while loop, H and W are being created here.

ct = CentroidTracker()


# Creating a object of Islemler to use in main while loop.
# necessary_process was created to use in while loop.

necessary_process = Operations()
#
#detection_graph, categories = loading_graphs.load_ssd_mobilenet()
...
#    
    def update_frame(self):
        ...
        YOLO_OBJECT.detection_people(_, self.image)
...
#        self.model = tf.keras.models.load_model("CNN-4")
...
#        self.sess.close()
    ...
    def detect_face(self,img):
        if radioButtonCam==0:
            rects, landmarks = face_detect.detect_face(img,10);
        else:
            rects, landmarks = face_detect.detect_face(img,10);#min face size is set to 80x80
        aligns = []
        positions = []
